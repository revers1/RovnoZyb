...
WORKDIR /src
COPY ["API_RovnoZyb/API_RovnoZyb.csproj", "API_RovnoZyb/"]
RUN dotnet restore "API_RovnoZyb/API_RovnoZyb.csproj"
COPY . .
WORKDIR "/src/API_RovnoZyb"
RUN dotnet build "API_RovnoZyb.csproj" -c Release -o /app/build

FROM build AS publish
RUN dotnet publish "API_RovnoZyb.csproj" -c Release -o /app/publish

FROM base AS final
WORKDIR /app
COPY --from=publish /app/publish .
ENTRYPOINT ["dotnet", "API_RovnoZyb+Angular.dll"]